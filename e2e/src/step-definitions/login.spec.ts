...
When('devo preencher o usuario {string}', { timeout: browser.pageLoadingTimeout }, async (name) => {
    await browser.actions().mouseMove(login.name).perform().then(() => {
      login.name.sendKeys(name);
    });
});
